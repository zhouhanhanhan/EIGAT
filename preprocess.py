...
def read_entity_from_id(filename='./data/NELL-995/entity2id.txt'):
    entity2id = {}
...
def read_relation_from_id(filename='./data/NELL-995/relation2id.txt'):
    relation2id = {}
    with open(filename, 'r') as f:
        for line in f:
            if len(line.strip().split()) > 1:
...
def load_filedata(filename, entity2id, relation2id, is_unweigted=False, directed=True):
...
def build_data(path='./data/NELL-995/', is_unweigted=False, directed=True):
    ...
    train_triples, train_adjacency_mat, unique_entities_train = load_filedata(os.path.join(
        path, 'train.txt'), entity2id, relation2id, is_unweigted, directed)
    validation_triples, valid_adjacency_mat, unique_entities_validation = load_filedata(
        os.path.join(path, 'valid.txt'), entity2id, relation2id, is_unweigted, directed)
    test_triples, test_adjacency_mat, unique_entities_test = load_filedata(os.path.join(
