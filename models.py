...
class SpGAT(nn.Module):
    def __init__(self, num_nodes, nfeat, nhid, relation_dim, dropout, alpha, d, nheads):
        """
    ...
    def forward(self, Corpus_, batch_inputs, entity_embeddings, relation_embed,
                edge_list, edge_type, edge_embed, edge_list_nhop, edge_type_nhop, entity_rank):
        x = entity_embeddings
        ...
        for att in self.attentions:
            h_prime, entity_rank = att(x, edge_list, edge_embed, edge_list_nhop, edge_embed_nhop, entity_rank, Corpus_)
...
#        result_1, result_2 = [att(x, edge_list, edge_embed, edge_list_nhop, edge_embed_nhop)
#                       for att in self.attentions]
#        x = torch.cat([result_1[0], result_2[0]], dim=1)
#        entity_rank = sum([result_1[1], result_2[1]])/self.nheads
        ...
        x, entity_rank = self.out_att(x, edge_list, edge_embed,
                               edge_list_nhop, edge_embed_nhop, entity_rank, Corpus_)
...
class SpKBGATModified(nn.Module):
    def __init__(self, initial_entity_emb, initial_relation_emb, entity_out_dim, relation_out_dim,
        ...
        self.sparse_gat_1 = SpGAT(self.num_nodes, self.entity_in_dim, self.entity_out_dim_1, self.relation_dim,
                                  self.drop_GAT, self.alpha, self.d, self.nheads_GAT_1)

        self.W_entities = nn.Parameter(torch.zeros(
            size=(self.entity_in_dim, self.entity_out_dim_1 * self.nheads_GAT_1)))
        nn.init.xavier_uniform_(self.W_entities.data, gain=1.414)

    def forward(self, Corpus_, adj, batch_inputs, train_indices_nhop, entity_rank):
        # getting edge list
        edge_list = adj[0]
        edge_type = adj[1]
